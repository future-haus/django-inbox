...
class UserViewSet(NestedMessagePreferencesMixin, mixins.RetrieveModelMixin, mixins.UpdateModelMixin, viewsets.GenericViewSet):
...
class UserDeviceViewSet(NestedViewSetMixin, mixins.CreateModelMixin, viewsets.GenericViewSet):
...
class DeviceViewSet(mixins.DestroyModelMixin, viewsets.GenericViewSet):
    ...
    def destroy(self, request, pk, *args, **kwargs):
        ...
        if response.status_code == 400 and response.json().get('error') == 'notification_key not found':
            device_group.delete()
